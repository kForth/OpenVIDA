    ...
    var JqAuto = function () {
        ...
        this.init = function (
            ...
            if (
                ...
                !ko.isObservable(options.source)
            ) {
                ...
                //process local data
                config.source = self.processOptions.bind(
                    self,
                    valueAccessor,
                    filter,
                    options.source
            ...
            existingSelect =
                typeof config.select === "function" && config.select;
            existingChange =
                typeof config.change === "function" && config.change;
            ...
            if (options.template) {
                widget._renderItem = self.renderItem.bind(
                    self,
                    options.template,
                    context
                );
        ...
        this.update = function (element, valueAccessor) {
            ...
            if ($.isArray(sources) && propNames.value) {
                value =
                    ko.utils.arrayFirst(sources, function (opt) {
        ...
        this.defaultFilter = function (item, term) {
            ...
            return (
                (item || item === 0) &&
                ko.toJSON(item).toLowerCase().indexOf(term) > -1
            );
        };
        ...
        this.getPropertyNames = function (valueAccessor) {
            ...
            return {
                ...
                input:
                    options.inputProp || options.labelProp || options.valueProp,
